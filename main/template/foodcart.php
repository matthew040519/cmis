     ...
     ?>
            ...
            <div class="container-fluid">
                ...
                <div class="row">
                   ...
                   <?php 
                            $sum = 0;
                            $query = mysqli_query($con, "SELECT *, tblfoods.price as food_price FROM `tblfoods` INNER JOIN tblorder ON tblorder.item_code=tblfoods.food_code WHERE tblorder.act_no = 0 AND tblorder.customer_id = '$customer_id' ORDER BY food_id DESC");
               ...
               ?>
                                ...
                                <div class="vertical-card__menu--desc p-3">
                                    <div class="d-flex justify-content-between">
                                        ...
                                        <div class="vertical-card__menu--fav">
                                            <a href="javascript:void()">Orig. Price: <?php echo number_format($row['food_price'], 2); ?></i></a>
                                        </div>
                                    </div>
                                    ...
                                    <div class="d-flex justify-content-between align-items-center">
                                        ...
                                        <div class="vertical-card__menu--rating c-pointer">
                                            <form method="POST" action="foodcart.php?id=<?php echo $row['order_id']; ?>">
                ...
                <?php 
                            // $sum = 0;
                            $query = mysqli_query($con, "SELECT *, tblevents.events_price as events_price FROM `tblevents` INNER JOIN tblorder ON tblorder.item_code=tblevents.event_code WHERE tblorder.act_no = 0 AND tblorder.customer_id = '$customer_id' ORDER BY events_id DESC");
                            while($row = mysqli_fetch_array($query)){
                                 $event_code = $row['event_code'];
                                 $active = $row['active'];
                                 $sum += $row['events_price'];
                    ?>
                        ...
                        <div class="card vertical-card__menu">
                            ...
                            <div class="card-header p-0">
                                <div class="vertical-card__menu--image">
                                    ...
                                    <img style="height: 300px;" src="../../images/events/<?php echo $row['events_image'] ?>" alt="Menu">
                                </div>
                            </div>
                            <div class="card-body p-0">
                                <div class="vertical-card__menu--desc p-3">
                                    <div class="d-flex justify-content-between">
                                        <h4 class="vertical-card__menu--title"><?php echo $row['events_desc']; ?></h4>
                                        <div class="vertical-card__menu--fav">
                                            <a href="javascript:void()">&#8369; <?php echo number_format($row['events_price'], 2); ?></i></a>
                                        </div>
                                    </div>
                                    <p>Qty: <?php echo $row['qty']; ?></p>
                                    <div class="d-flex justify-content-between align-items-center">
                                        <h2 class="vertical-card__menu--price">&#8369;<span><?php echo number_format($row['events_price'], 2); ?></span></h2>
                                        <div class="vertical-card__menu--rating c-pointer">
                                            <form method="POST" action="foodcart.php?id=<?php echo $row['order_id']; ?>">
                                                <input type="submit"  class="btn btn-danger" value="Remove" name="remove">
                                            </form>
                ...
                <?php 
                            // $sum = 0;
                            $query = mysqli_query($con, "SELECT * from tblfoods INNER JOIN tblgiveaways on tblgiveaways.food_id=tblfoods.food_id WHERE '$sum' BETWEEN tblgiveaways.begin_price AND tblgiveaways.limit_price");
                            while($row = mysqli_fetch_array($query)){
                                 $food_code = $row['food_code'];
                                 $active = $row['active'];
                                 // $sum += $row['price'];
                    ...
                    <div class="col-xl-4 col-sm-4 col-xxl-4">
                        <div class="card vertical-card__menu">
                           <span class="ribbon ribbon__one vertical-card__menu--status closed">Freebies <em class="ribbon-curve"></em></span>
                            <div class="card-header p-0">
                                <div class="vertical-card__menu--image">
                                    <?php 
                  $queryimage = mysqli_query($con, "SELECT *, MAX(fimage_id) as fimage_id FROM `tblfoodimage` WHERE food_code = '$food_code'");
                            ...
                            <div class="card-body p-0">
                                <div class="vertical-card__menu--desc p-3">
                                    <div class="d-flex justify-content-between">
                                        <h4 class="vertical-card__menu--title"><?php echo $row['food_desc']; ?></h4>
                                        <div class="vertical-card__menu--fav">
                                           <!--  <a href="javascript:void()">Orig. Price: <?php echo number_format($row['price'], 2); ?></i></a> -->
                                        </div>
                                    </div>
                                    <p>Qty: <?php echo $row['qty']; ?></p>
                                    <div class="d-flex justify-content-between align-items-center">
        ...
        {
            $code = $_GET['id'];
                    $deleteQuery = mysqli_query($con, "DELETE FROM tblorder WHERE order_id = '$code'");
...
// echo $selected."</br>";
    $query1 = mysqli_query($con, "SELECT *, tblfoods.price as food_price FROM `tblfoods` INNER JOIN tblorder ON tblorder.item_code=tblfoods.food_code WHERE tblorder.act_no = 0 AND tblorder.customer_id = '$customer_id' ORDER BY food_id DESC");
              ...
              $query1 = mysqli_query($con, "SELECT *, tblevents.events_price as events_price FROM `tblevents` INNER JOIN tblorder ON tblorder.item_code=tblevents.event_code WHERE tblorder.act_no = 0 AND tblorder.customer_id = '$customer_id' ORDER BY events_id DESC");
              while ($row = mysqli_fetch_array($query1)){
                $order_id1 = $row['order_id'];
                $updatequery = mysqli_query($con, "UPDATE `tblorder` SET `order_code`='$randomString',`act_no`=1 WHERE order_id = $order_id1");
              }

              $query = mysqli_query($con, "INSERT INTO `tblnotification`(`notification_id`, `customer_id`, `transaction_id`) VALUES (NULL, '$customer_id', '$randomString')");
            // }
            echo "<script>window.location.replace('foodcart.php')</script>";
        }
     ?>
